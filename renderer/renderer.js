...
// Milliseconds per frame. Change these numbers to tune each animation separately.
...
function setInitial(state) {
  initial = state;
  // Selecting a state is not a fresh hover. Keep Jump disabled until the pointer leaves.
  suppressHover = true;
  hoverArmed = false;
}
...
window.petHost.onMenuClosed(() => {
  menuOpen = false;
  if (!suppressHover) {
    suppressHover = pet.matches(':hover');
    hoverArmed = !suppressHover;
  }
  restoreInitial();
});
